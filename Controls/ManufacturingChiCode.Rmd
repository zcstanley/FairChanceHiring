...
# Linear Models and Residual Analysis of MANUFACTURING
...
lm_fairchance_controls <- lm(employment_data$avg_manufacturing ~ control_data$macroeconomy + control_data$socioeconomics + control_data$politics + control_data$diversity + employ_z_scores$manu_employment_z)

# Save the residuals of the model to employment_data$fair_chance_rate_residuals
employment_data$manufacturing_residuals <- residuals(lm_fairchance_controls)

# Print the summary of the linear model
summary(lm_fairchance_controls)

# Run a linear model for the total positive index vs the 5 types of control_data
lm_model_pos_index <- lm(pol_indices$tot_pos_index ~ control_data$macroeconomy + control_data$socioeconomics + control_data$politics + control_data$diversity + employ_z_scores$manu_employment_z)
...
plot_residual_pos_vs_fairchance <- ggplot(data = data.frame(y = employment_data$manufacturing_residuals, x = pol_indices$pos_index_residuals)) +
  ...
  labs(
    ...
    y = "Residualized Fair Chance Job Posting Rate, manufacturing + Food Service",
...
employment_above_avg <- ifelse(employment_data$manufacturing_residuals > 0, "Above Avg", "Below Avg")
...
# Run a linear model for the total positive index vs the 5 types of control_data
lm_model_neg_index <- lm(pol_indices$neg_combo_index ~ control_data$macroeconomy + control_data$socioeconomics + control_data$politics + control_data$diversity + employ_z_scores$manu_employment_z)
...
```{r scatter-residual-pos-fairchance, echo=TRUE}
# Scatter plot of residualized positive policy vs. residualized fair chance job posting rate
plot_residual_neg_vs_fairchance <- ggplot(data = data.frame(y = employment_data$manufacturing_residuals, x = pol_indices$neg_index_residuals)) +
  geom_point(aes(x = x, y = y)) +
  geom_smooth(aes(x = x, y = y), method="lm", se=FALSE)+
  labs(
