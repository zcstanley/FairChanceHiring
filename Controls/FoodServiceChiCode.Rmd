...
# Linear Models and Residual Analysis of FOOD SERVICE
...
lm_fairchance_controls <- lm(employment_data$avg_food_service ~ control_data$macroeconomy + control_data$socioeconomics + control_data$politics + control_data$diversity + employ_z_scores$arts_employment_z)

# Save the residuals of the model to employment_data$fair_chance_rate_residuals
employment_data$food_service_residuals <- residuals(lm_fairchance_controls)
...
plot_residual_pos_vs_fairchance <- ggplot(data = data.frame(y = employment_data$food_service_residuals, x = pol_indices$pos_index_residuals)) +
  ...
  labs(
    ...
    y = "Residualized Fair Chance Job Posting Rate + Food Service",
...
employment_above_avg <- ifelse(employment_data$food_service_residuals > 0, "Above Avg", "Below Avg")
...
```{r scatter-residual-pos-fairchance, echo=TRUE}
# Scatter plot of residualized positive policy vs. residualized fair chance job posting rate
plot_residual_neg_vs_fairchance <- ggplot(data = data.frame(y = employment_data$food_service_residuals, x = pol_indices$neg_index_residuals)) +
  geom_point(aes(x = x, y = y)) +
  geom_smooth(aes(x = x, y = y), method="lm", se=FALSE)+
  labs(
