...
# Load the variables for the 2022 ACS 5-year data, S1903 table
variables_s1903 <- load_variables(2022, "acs5/subject", cache = TRUE)

# View the variables in RStudio's data viewer
View(variables_s1903)

# Filter for the median household income variable
median_income_vars <- variables_s1903 %>%
  filter(str_detect(label, "Median income") & str_detect(label, "households"))

print(median_income_vars)
...
```{r}
variables_s1501 <- load_variables(2022, "acs5/subject", cache = TRUE)

education_vars <- variables_s1501 %>%
  filter(str_detect(label, "high")& str_detect(name, "0501"))
```


```{r}
# Load variables from the DP03 table (part of the ACS 5-year data profile tables)
variables_dp03 <- load_variables(2022, "acs5/profile", cache = TRUE)

employment_vars <- variables_dp03 %>%
  filter(str_detect(label, "labor"))
...
```{r}
library(tidyverse)
ctrl_vars <- read_csv("control_vars_clean.csv")
```


```{r}

ctrl_vars <- ctrctrl_vars <- ctrl_vars %>%
...
clean_vars <- ctrl_vars %>%
  mutate(
    ...
    prob_complete_rate = as.numeric(prob_complete_rate),
    parole_complete_rate = as.numeric(parole_complete_rate),
    rev_rate = as.numeric(rev_rate)
  )
```
